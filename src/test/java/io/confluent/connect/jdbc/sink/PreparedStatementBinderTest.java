...
public class PreparedStatementBinderTest {
  ...
  public void bindRecord() throws SQLException, ParseException {
    ...
    int date = Date.fromLogical(Date.SCHEMA, (java.util.Date) valueStruct.get("date"));
    verify(statement, times(1)).setDate(index++, new java.sql.Date(date));
    verify(statement, times(1)).setInt(index++, Time.fromLogical(Time.SCHEMA, (java.util.Date) valueStruct.get("time")));

    long ts = Timestamp.fromLogical(Timestamp.SCHEMA, (java.util.Date) valueStruct.get("timestamp"));
    verify(statement, times(1)).setTimestamp(index++, new java.sql.Timestamp(ts));
